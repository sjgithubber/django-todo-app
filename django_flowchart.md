...
flowchart TB
    ...
    subgraph example["Blog App Example (post_detail)"]
        ex1["User requests /blog/post/1/"]
        ex2["urls.py matches to post_detail view"]
        ex3["View calls Post.objects.get(pk=1)"]
        ex4["ORM translates to SQL: SELECT * FROM blog_post WHERE id=1"]
        ex5["Database returns post data"]
        ex6["View renders blog/post_detail.html with post data"]
        ex7["Response sent to browser"]
    end
